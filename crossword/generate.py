...
class CrosswordCreator():
    ...
    def consistent(self, assignment):
        ...
        for s in assignment:
            ...
            if list(assignment.values()).count(assignment[s]) >= 2:
                return False
            
            # Return False if the assignment has a wrong length
            if len(assignment[s]) != s.length:
                return False

        for X in assignment.keys():
            for Y in self.crossword.neighbors(X):
                if Y in assignment.keys():
                    # Calculates the overlap for the neighbors (X,Y)
                    overlap = self.crossword.overlaps[X,Y]
                    # print(assignment[X])
                    # print(assignment[Y])
                    if overlap is not None:
                        # Check if the overlap of the assignment is not correct
